...
"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/health") as response:
                    if response.status == 200:
                        ...
                        print(
                            f"{Colors.RED}✗ API health check failed (HTTP {response.status}){Colors.END}"
                        )
                        return False
    ...
    def format_tools_used(self, tools: List[Dict[str, Any]]) -> str:
        ...
        for i, tool in enumerate(tools, 1):
            ...
            if args:
                ...
                if "query" in args:
                    key_args.append(
                        f"query='{args['query'][:50]}{'...' if len(args['query']) > 50 else ''}'"
                    )
                if "limit" in args:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        print(
                            f"{Colors.RED}✗ API Error ({response.status}): {error_text}{Colors.END}"
                        )
                        return
    ...
    async def run(self):
        ...
        if not await self.check_health():
            print(
                f"{Colors.RED}Cannot connect to API. Please ensure the server is running.{Colors.END}"
            )
            return

        print(
            f"{Colors.GREEN}Ready to chat! Ask me about tech companies and AI initiatives.{Colors.END}\n"
        )
